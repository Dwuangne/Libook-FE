...
import { IconButton, Box, Typography, Paper, Button, CircularProgress } from '@mui/material';
...
const ChatBox = () => {
  ...
  const fetchConversationById = useCallback(async (conversationId, isAppending = false, currentPageIndex = pageIndex) => {
    ...
    try {
      ...
      const messageRes = await GetMessageByConversationIdApi(conversationId, currentPageIndex, pageSize);
  ...
  const joinConversations = async () => {
    ...
    if (connection.state === signalR.HubConnectionState.Connected) {
      try {
        ...
        console.error(`Error joining conversation ${selectedConversation}:`, err);
      }
    } else {
...
const handleSendMessage = async () => {
  ...
  try {
    if (!selectedConversation) {
      ...
      await connection.invoke('SendMessage', selectedConversation, trimmedInput, userId);
    }
    setInput("");
  } catch (err) {
